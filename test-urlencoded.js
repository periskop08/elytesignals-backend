...
async function run() {
  ...
  const url = "https://open-api.bingx.com/openApi/swap/v2/trade/order?signature=" + sign1 + "&" + rawQs;
  try {
     const res = await axios.post(url, null, { headers: {"X-BX-APIKEY": API_KEY, "Content-Type":"application/x-www-form-urlencoded"} });
     console.log("URLEncoded JSON:", res.data);
  } catch(e) { console.error("Error URLEncoded:", e.response?.data); }
}
run();
