...
async function getTopPairsBingX(limit) {
    try {
        ...
        // Yeni Kural: Hacim tavanını 2.5 Milyona indir!
        const usdtPairs = list.filter(item => item.symbol.endsWith('-USDT') && parseFloat(item.quoteVolume) > 2500000);
        usdtPairs.sort((a,b) => parseFloat(b.quoteVolume) - parseFloat(a.quoteVolume));
        
        // Return object array {symbol, volume}
...
async function backtest(assetInfo) {
    ...
    for (let i = 100; i < candles.length - 24; i++) {
        ...
        if (direction === 'LONG' && globalVol < 10000000) {
            continue; 
        }
        if (direction === 'SHORT' && globalVol < 2500000) {
            continue; // Hacim 2.5 Milyon altındaysa short yasak!
        }
...
async function run() {
    process.stdout.write("Fetching top BingX pairs for 1 month Dual Liquidity backtest...\n");
    // Limit to 300 pairs with volume > 2.5M (will include both solid and shitcoins)
    ...
    for (let i = 0; i < pairs.length; i += batchSize) {
        ...
        results.forEach(res => {
            ...
            res.trades40.forEach(t => { 
                ...
                if(t.outcome === 'WIN') stats.tp++;
                else if(t.outcome === 'LOSS') stats.sl++;
                else stats.pending++;
            });
        });
    ...
    const output = {
        Strategy: "Dual Liquidity System (Long:10M+, Short:2.5M+)",
        Period: "1 Month (720h)",
        QualityThreshold: ">= 40",
    ...
    console.log("\n=== DUAL LIQUIDITY BACKTEST RAPORU ===");
