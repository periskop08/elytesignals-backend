...
// FETCH TOP PAIRS BYBIT
async function getTopPairsBybit() {
    try {
        const res = await axios.get('https://api.bybit.com/v5/market/tickers?category=linear');
        const list = res.data.result.list;
        return list.filter(item => item.symbol.endsWith('USDT') && parseFloat(item.turnover24h) > 10000000).map(i => i.symbol);
    } catch (e) { return []; }
}

// FETCH TOP PAIRS BINGX
async function getTopPairsBingX() {
    try {
        const res = await axios.get('https://open-api.bingx.com/openApi/swap/v2/quote/ticker');
        const list = res.data.data;
        return list.filter(item => item.symbol.endsWith('-USDT') && parseFloat(item.quoteVolume) > 10000000).map(i => i.symbol);
    } catch (e) { return []; }
}
...
async function simulateSignal(candles, symbol, platform) {
    ...
    for (let i = 100; i < candles.length - 24; i++) {
        ...
        trades55.push({ symbol, platform, date: candles[i].date, dateStr: new Date(candles[i].date).toLocaleString(), direction, score: qualityScore, outcome });
        i += 6; 
    }
...
async function runComparitive() {
    process.stdout.write("Fetching top pairs from both exchanges...\n");
    const bybitSymbols = await getTopPairsBybit();
    const bingxSymbols = await getTopPairsBingX();
    
    // Normalize and intersect
    const bybitMap = new Set(bybitSymbols);
    const commonSymbols = bingxSymbols.filter(s => bybitMap.has(s.replace('-', '')));
    
    process.stdout.write(`Found ${commonSymbols.length} mutually common high-volume pairs.\nStarting comparative backtest for common pairs on Elite (55+) score...\n`);
    
    let statsBybit = { trades: 0, win: 0, loss: 0 };
    let statsBingx = { trades: 0, win: 0, loss: 0 };
    
    const batchSize = 5;
    for (let i = 0; i < commonSymbols.length; i += batchSize) {
        const batch = commonSymbols.slice(i, i + batchSize);
        
        const promises = batch.map(async (bingxSym) => {
            const bybitSym = bingxSym.replace('-', '');
            
            const [candlesBybit, candlesBingx] = await Promise.all([
                fetchBybitCandles(bybitSym, 1000),
                fetchBingxCandles(bingxSym, 1000)
            ]);
            
            const tradesBybit = await simulateSignal(candlesBybit, bybitSym, 'BYBIT');
            const tradesBingx = await simulateSignal(candlesBingx, bingxSym, 'BINGX');
        ...
        results.forEach(res => {
            res.bybit.forEach(t => { t.outcome === 'WIN' ? statsBybit.win++ : (t.outcome === 'LOSS' ? statsBybit.loss++ : null) });
            res.bingx.forEach(t => { t.outcome === 'WIN' ? statsBingx.win++ : (t.outcome === 'LOSS' ? statsBingx.loss++ : null) });
            statsBybit.trades += res.bybit.length;
            statsBingx.trades += res.bingx.length;
        });
        
        process.stdout.write(`Processed ${Math.min(i + batchSize, commonSymbols.length)}/${commonSymbols.length} pairs...\n`);
    }
    
    const output = {
        commonPairsCount: commonSymbols.length,
        bybit: {
            totalTrades: statsBybit.trades,
            wins: statsBybit.win,
            losses: statsBybit.loss,
            winRate: statsBybit.win+statsBybit.loss>0 ? ((statsBybit.win/(statsBybit.win+statsBybit.loss))*100).toFixed(1) : 0
        },
        bingx: {
            totalTrades: statsBingx.trades,
            wins: statsBingx.win,
            losses: statsBingx.loss,
            winRate: statsBingx.win+statsBingx.loss>0 ? ((statsBingx.win/(statsBingx.win+statsBingx.loss))*100).toFixed(1) : 0
        }
    };
    
    console.log("=== APPLES-TO-APPLES COMMON PAIR COMPARISON ===");
