...
### Tarih: 11 Nisan 2026 - PeriskopAI Dinamik Otopilot ve Risk Yönetimi
**(Sürüm: v3.2.0 - Otonom Risk Modeli)**
*   **(Market Regime):** Sadece "fırsat" değil "Piyasa Rüzgarı" takip eklendi. ADX ve ATR volatilitesinin gücüne göre otopilot kendini Trending (Bol fırsatlı, +5 puan destekli) veya Ranging (Testere, -5 ceza sınırlaması) moduna sokur.
*   **(Dinamik Kasa - Dynamic Sizing):** Arka arkaya alınan 2 Stop'ta (Loss) sistem zararı kısmak için bütçe çarpanını **0.5x** defansa çekerken, arka arkaya alınan 2 Hedef (Win) serisinde kasa riskini **1.5x** agresifleştirerek momentumdan maksimum nakit sağmaya programlandı.
...
*   **(AI Core) Hedge Fon Ajanları Kişiselleştirmesi:** Sistem arkasında koşan düz ve isimsiz kod blokları, "Elyte Ajan Bürokrasisi"ne dönüştürüldü. Her ajanın Telegram'da kimliği ve sabit mesaisi kodlandı: 
    ...
    - **Demir Bey:** Likidite, Sipariş Derinliği (Order Book) ve Kayma (Slippage) Yöneticisi (`demir_bey.js`).
...
*   **(Demir Bey Soft-Fail Koruması - FOK Extension):** Altay Bey'in ürettiği sinyal yayınlanmadan hemen önce (Kalite Skoru > 55 ise), fonun piyasaya gireceği tahtanın likiditesi 2000 milisaniyelik bir zaman aşımı (Timeout) kalkanıyla Demir Bey'e sorulur. Eğer tahta sığ veya spread %0.4'ten genişse Demir Bey işlemi -15 puanla iptal eder (Veto). Eğer Demir Bey çökerse, asıl sistem çökmez ve "Bypass" edilerek sinyal yayınlanır. Emir iletimindeki bu otonom kalkan, PnL sızıntılarını durdurur.
...
### Tarih: 14 Nisan 2026 - R:R Hybrid Fix & Otonom Gölge İstihbaratı
**(Sürüm: v4.1.0 - Alpha/Volume Stabilizasyonu)**
*   **(Slippage Kalkanının Otonomizasyonu):** İşlemlerin R:R'sini (Risk-Ödül Oranı) hesaplarken yapılan statik `%15 Kayma Payı (Slippage)` maliyeti eklentisi, kalite puanlamasını haksız yere düşürdüğü için tamamen kaldırıldı (Hybrid Fix). Slippage koruması R:R hesabından çıkarılarak, doğrudan icra (Execution) aşamasındaki `Demir Bey` kalkanına ve borsa emir kontrol mekanizmalarına devredildi.
*   **(Dinamik R:R Baraj Optimizasyonu):** Slippage esnemelerinden sonra hacim (Volume) ve nitelikli işlemler (Alpha) barajı sistemin nefes alıp fırsatları yakalayabilmesi adına esnetildi. Yeni limitler: **Alpha için 1.22, Volume için 1.10** R:R olarak belirlenmiştir.
*   **(Börü Bey - ADX Gölge Takibi):** Piyasada aşırı alım/satım (StochRSI Şişkin) olan ve ADX seviyesi 24'ün altında bulunduğu için "Zayıf Trend / Manipülatif Alan" tespitiyle `[VETO]` yiyen sinyaller, artık tamamen veritabanından silinmek yerine sanal veritabanına (`shadow_trades`) gönderiliyor. İşlemi reddeden Sistem Vetosunun (%100 matematiksel kuralın) başarısı, gece raporlamasında test ediliyor. Bu sinyaller hedefe ulaşırsa (WIN) Börü Bey, "Sabit kural yüzünden para kaçırdık" diyerek özel Telegram uyarısı geçiyor. 
*   **(Börü Bey - R-Expectancy Kasa Büyüme Tablosu):** Gece 03:20 Telegram Raporu'na tam teçhizatlı bir R-Expectancy (Net R getiri potansiyeli) raporu eklendi. Ana sistemin kasasının yanısıra Börü Bey'in çöpten kurtardığı simüle işlemlerin getireceği Teorik Kasa Büyümesi (%) ölçülerek her iki motor karşılaştırmalı test ediliyor.
---
