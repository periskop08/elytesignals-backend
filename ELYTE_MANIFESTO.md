...
*   **(Frontend Veri API Onarımı):** FET (ASI birleşmesi) gibi coinlerin borsa listelemelerinden kopması ve fiyatı `undefined` olan kripto varlıkların canlı PnL hesaplayamama pürüzleri giderildi. Frontend'deki fiyat çekmecesi `Bybit API`'den, Backend ile %100 senkron olması için `BingX Swap API` üzerine taşındı.
