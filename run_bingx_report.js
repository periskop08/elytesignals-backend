...
async function backtest(symbol) {
    ...
    for (let i = 100; i < candles.length - 24; i++) {
        ...
        if (qualityScore < 30) continue; // SCORE THRESHOLD CHANGED TO 30
        ...
        trades30.push({ date: new Date(candles[i].date).toLocaleString(), direction, score: qualityScore, outcome });
        i += 6; 
    }
    
    return { symbol, trades30 };
}

async function run() {
    process.stdout.write("Fetching top BingX pairs for 1 month backtest...\n");
    const pairs = await getTopPairsBingX(50); // Limit to top 50 to speed up
    ...
    console.log("\n=== 1 AYLIK BINGX (SKOR:30) BACKTEST RAPORU ===");
