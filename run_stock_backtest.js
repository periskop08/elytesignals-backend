...
async function backtest(symbol) {
    ...
    // 6 Months = ~180 Days -> roughly 900 trading hours (comparable to 1 month of crypto)
    let candles = await fetchYF(symbol, 180);
    if (!candles || candles.length < 150) return null;
    
    // Slicing to roughly 1 month (approx last 150-180 candles)
    let trades = [];
    
    for (let i = 80; i < candles.length - 10; i++) {
        const window = candles.slice(i - 80, i);
        ...
        // For standard filter test, require 60 points (Elite)
        ...
        for (let f = i; f < candles.length; f++) {
            if (direction === 'LONG') {
                if (candles[f].low <= dynamicStop) { outcome = 'LOSS'; outcomePrice = dynamicStop; break; }
                if (candles[f].high >= targetP) { outcome = 'WIN'; outcomePrice = targetP; break; }
            } else {
                if (candles[f].high >= dynamicStop) { outcome = 'LOSS'; outcomePrice = dynamicStop; break; }
                if (candles[f].low <= targetP) { outcome = 'WIN'; outcomePrice = targetP; break; }
            }
        }
        
        trades.push({
            date: new Date(candles[i].date).toLocaleString(),
            direction, score: qualityScore, outcome
        });
        i += 5; // cooldown
    }
