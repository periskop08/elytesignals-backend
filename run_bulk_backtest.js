...
async function getTopPairs(limit) {
    try {
        ...
        return ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'XRPUSDT', 'SUIUSDT']; // fallback
    }
}

async function backtest(symbol) {
    let candles = await fetchBybitCandles(symbol, 60, 1000);
    if (!candles || candles.length < 200) return null;
    
    let trades40 = [];
     let trades55 = [];
    
    // Simulate window scanning
...
async function run() {
    process.stdout.write("Fetching top 100 coins...\n");
    const pairs = await getTopPairs(100);
    process.stdout.write(`Found ${pairs.length} pairs. Starting bulk backtest (this might take a few seconds)...\n`);
    ...
    console.log("=== BULK BACKTEST RESULTS ===");
