...
async function getUsdtPairsAndAssets() {
    try {
        ...
        const ignoredStables = ['AAPLX-USDT', 'USDC-USDT', 'USD1-USDT', 'USDE-USDT', 'BUSD-USDT', 'TUSD-USDT', 'FDUSD-USDT', 'EUR-USDT', 'DAI-USDT', 'USTC-USDT', 'PYUSD-USDT', 'CRCLX-USDT', 'NXPC-USDT', 'PAXG-USDT', 'XAUT-USDT'];
...
Eğer derslerden biriyle doğrudan çelişmiyorsa sadece "ONAY" yaz.`;
                ...
                const insertResult = await db.run(
                    "INSERT INTO signals (symbol, type, entryPrice, targetPrice, stopPrice, qualityScore, warnings, rvol, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    [signal.symbol, signal.type, signal.entryPrice, signal.targetPrice, signal.stopPrice, signal.qualityScore, signal.warnings, volumeTextForDb, signal.status]
                );
